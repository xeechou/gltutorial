...
GLuint
TT::loadTexture2GPU(const std::string fname)
{
	GLuint tid;
...
TT::Mesh::Mesh(const aiScene *scene, aiMesh *mesh)
{
...
TT::Mesh::Mesh(const std::vector<glm::vec3>& vertxs,
...
TT::Mesh::pushMesh2GPU(void)
{
	...
	glBufferData(GL_ARRAY_BUFFER, this->vertices.size() * sizeof(TT::Vertex), //size
...
void
TT::Mesh::draw(GLuint prog, const Model& model)
{
	glUseProgram(prog);
	...
	for (GLuint i = 0; i < mat.size(); i++) {
		...
		if (mat[i].type == TT::Texture::Diffuse)
			uniform = std::string("diffuse");
		else if (mat[i].type == TT::Texture::Specular)
...
int
TT::Model::processNode(const aiScene *scene, aiNode *node)
{
 ...
 * In terms of 
 */
TT::Model::Model(const std::string& file)
{
