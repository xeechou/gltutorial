...
int main(void)
{
	...
	//This is what you used to load transparent textures
	cv::Mat img = cv::imread("../imgs/grass.png", CV_LOAD_IMAGE_UNCHANGED);
	GLuint grass;
...
//	glUniform1i(glGetUniformLocation(prog_id, "material.specular"), 0);
	...
	ShaderMan blend1("vs.glsl", "blendfs.glsl");
	GLuint blend_id = blend1.getPid();
...
//			  << light_pos[2] << "]" << std::endl;
		...
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
		glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST_MIPMAP_NEAREST);
		glUniform1i(glGetUniformLocation(prog_id, "diffuse"), 0);
		glActiveTexture(0);
...
//		glDrawArrays(GL_TRIANGLES, 0, 36);
		...
		glUniform1i(glGetUniformLocation(blend_id, "blendtexture"), 1);
